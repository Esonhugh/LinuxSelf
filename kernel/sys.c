...
struct prcs_info {
	...
	atomic_t nscount; 		// namespace count
	unsigned long nvcsw; 		// context switch count
	u64 start_time; 	// start time in ns
};
SYSCALL_DEFINE2(esoncall, struct prcs_info __user *, pf, pid_t, pid) 
{
	...
	if (!task) {
		printk(KERN_ALERT"esoncall(): Invalid pid or no such process \n");
		return -EINVAL;
	}
	
	pif.prio = task->prio;
	pif.state = task->__state;
	pif.nscount = task->nsproxy->count;
	pif.nvcsw = task->nvcsw;
	pif.start_time = task->start_time;
	
	if (copy_to_user(pf, &pif, sizeof(struct prcs_info))) {
		printk(KERN_ALERT"esoncall(): Copy to user fail");
		return -EFAULT;
	}
